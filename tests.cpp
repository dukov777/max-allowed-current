...
TEST(CapacitorTest, TestCapacitor) {
    double f = 1;
    Capacitor cap(1, 1000, 500, 500e3);
...
TEST(CapacitorTest, TestCapacitorDecoratorExceedingRange) 
{
    ...
    f = 50;
    ASSERT_THROW(decoratedCap.voltage(f, 1001), std::runtime_error);
}
...
TEST(CapacitorTest, TestParallelCapacitorsAllowedMaxCurrent) {
    double f = 1;
    Capacitor cap1(10, 800, 500, 500e3);
    Capacitor cap2(10, 1000, 600, 500e3);
    ...
    ASSERT_NEAR(parallel.allowed_current(f), expected_current, 1e-4);
}
...
TEST(CapacitorTest, TestSeriesCapacitorsAllowedMaxCurrent) {
    ...
    ASSERT_NEAR(serial.allowed_current(f), expected_current, 1e-4);
}
